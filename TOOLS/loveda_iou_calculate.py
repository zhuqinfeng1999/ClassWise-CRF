...
gt_dir = "/ZQFSSD/crf/DATA/loveda/lovedagt"  # Directory containing ground truth images
pred_dir = "/ZQFSSD/crf/OUTPUT/LOVEDA/NPY/4_opt_weight_e2.5/trial_7"  # Directory containing predicted images
...
def calculate_iou_miou(gt_dir, pred_dir):
    """
    Calculate IoU and mIoU for all images in the given directories using GPU.

    Args:
        gt_dir (str): Path to directory with ground truth images
        pred_dir (str): Path to directory with predicted images
    ...
    for gt_file, pred_file in zip(gt_files, pred_files):
        assert gt_file == pred_file, f"File names do not match: {gt_file} vs {pred_file}"
...
if __name__ == "__main__":
    # Calculate IoU and mIoU using the hardcoded directories
    iou, miou = calculate_iou_miou(gt_dir, pred_dir)
