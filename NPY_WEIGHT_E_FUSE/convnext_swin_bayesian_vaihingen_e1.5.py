...
# 配置日志记录
log_file_path = r"/ZQFSSD/crf/OUTPUT/vaihingen/NPY/swint_convnextt_opt_weight_e1.5/swint_convnextt_opt.log"
...
# 2. 路径配置
net1_dir = r"/ZQFSSD/crf/DATA/vaihingen/convnexttpre_npy"  # 网络1预测结果 (.npy)
net1_dir = r"/ZQFSSD/crf/DATA/vaihingen/convnexttpre_npy"  # 网络1预测结果 (.npy)
net2_dir = r"/ZQFSSD/crf/DATA/vaihingen/swintpre_npy"  # 网络2预测结果 (.npy)
images_dir = r"/ZQFSSD/crf/DATA/vaihingen/ori"      # 原始图像
gt_dir = r"/ZQFSSD/crf/DATA/vaihingen/vaihingengt"     # 地面真相
output_dir = r"/ZQFSSD/crf/OUTPUT/vaihingen/NPY/swint_convnextt_opt_weight_e1.5"  # CRF优化结果
...
iou_net1 = np.array([83.35, 90.65, 68.75, 82.63, 72.57, 49.68])  # ConvNext-T
iou_net2 = np.array([84.00, 91.16, 68.55, 82.67, 72.03, 50.36])  # Swin-T

# 超参数 alpha，用于控制指数加权的强度
alpha = 1.5  # 可根据实验调整，例如 1.5, 2.0, 3.0
...
def objective(trial):
    ...
    args_list = [
        (os.path.join(net1_dir, f), os.path.join(net2_dir, f), os.path.join(images_dir, f.replace('.npy', '.tif')),
         os.path.join(temp_output_dir, f.replace('.npy', '.tif')), gaussian_sdims, gaussian_compat, bilateral_sdims, bilateral_schan, bilateral_compat, weights_net1, weights_net2)
...
# Compute and log per-class IoUs and mIoU for the best trial
    ...
    ious, miou = compute_miou_for_dir(gt_dir, best_output_dir, PALETTE, DEVICE)
    logging.info("\nPer-class IoUs:")
