...
# Configure logging
log_file_path = r"/ZQFSSD/crf/OUTPUT/vaihingen/NPY/3_opt_weight_e2.5/3_opt_weight_e2.5.log"
...
net1_dir = r"/ZQFSSD/crf/DATA/vaihingen/vmambatpre_npy"  # Network 1 predictions (.npy)
net2_dir = r"/ZQFSSD/crf/DATA/vaihingen/swintpre_npy"    # Network 2 predictions (.npy)
net3_dir = r"/ZQFSSD/crf/DATA/vaihingen/convnexttpre_npy" # Network 3 predictions (.npy)
images_dir = r"/ZQFSSD/crf/DATA/vaihingen/ori"            # Original images
gt_dir = r"/ZQFSSD/crf/DATA/vaihingen/vaihingengt"        # Ground truth
output_dir = r"/ZQFSSD/crf/OUTPUT/vaihingen/NPY/3_opt_weight_e2.5"  # CRF optimized results
...
iou_net1 = np.array([83.39, 91.11, 68.91, 83.04, 70.46, 49.51])  # VMamba-T
iou_net2 = np.array([84.00, 91.16, 68.55, 82.67, 72.03, 50.36])  # Swin-T
iou_net3 = np.array([83.35, 90.65, 68.75, 82.63, 72.57, 49.68])  # ConvNext-T
...
def objective(trial):
    ...
    args_list = [
        (os.path.join(net1_dir, f), os.path.join(net2_dir, f), os.path.join(net3_dir, f),
         os.path.join(images_dir, f.replace('.npy', '.tif')),
         os.path.join(temp_output_dir, f.replace('.npy', '.tif')),
...
if __name__ == "__main__":
    ...
    # Compute and log per-class IoUs and mIoU for the best trial
    best_output_dir = os.path.join(output_dir, f"trial_{best_trial.number}")
    ious, miou = compute_miou_for_dir(gt_dir, best_output_dir)  # Corrected call without PALETTE, DEVICE
    logging.info("\nPer-class IoUs:")
    for cls, iou in zip(CLASSES, ious):
        logging.info(f"  {cls}: {iou:.4f}")
